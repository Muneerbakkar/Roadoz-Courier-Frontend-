...
import { getFranchises, deleteFranchise, toggleFranchiseStatus } from "../redux/franchiseSlice";
...
export function Franchise() {
  ...
  const handleToggleStatus = async (franchise) => {
    try {
      await dispatch(toggleFranchiseStatus({ 
        id: franchise.id, 
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `franchise_registry_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="space-y-6 pb-20 p-4 max-w-7xl mx-auto">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-text-muted text-sm animate-pulse">Loading registry data...</p>
        </div>
      ) : (
        <>
          {/* Desktop View */}
                      ...
                      <tr key={f.id} className="hover:bg-dashboard-bg/20 transition-colors group">
                        ...
                        <td className="px-6 py-4 space-y-1">
                           <div className="flex items-center gap-1.5 text-[11px] text-text-muted">
                             <Mail size={12} className="text-primary/70"/> {f.email_id}
                           </div>
                           <div className="flex items-center gap-1.5 text-[11px] text-text-muted">
                             <Phone size={12} className="text-primary/70"/> {f.mobile_number}
                           </div>
                        </td>
                        <td className="px-6 py-4">
                           <div className="flex items-center gap-1 text-xs font-bold text-text-main uppercase">
                             <MapPin size={12} className="text-primary"/> {f.proposed_location}
                           </div>
                           <div className="text-[10px] text-text-muted ml-4 font-mono">PIN Covered: {f.pin_codes_covered?.split(',')[0]}...</div>
                        </td>
                        <td className="px-6 py-4">
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-text-main">{f.full_name}</p>
                        <p className="text-[10px] text-text-muted uppercase tracking-wider">{f.proposed_location}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2 py-3 border-y border-border-subtle/50">
                      ...
                      <div className="flex items-center gap-2 text-text-muted">
                        ...
                        <span className="text-[11px] truncate">{f.email_id}</span>
                      </div>
                    </div>
