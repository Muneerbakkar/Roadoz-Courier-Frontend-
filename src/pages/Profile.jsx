...
import { fetchProfile, updateProfile, uploadProfileImage } from "../redux/profileSlice"; 
import {
    User, Mail, Phone, MapPin, Camera,
    Shield, Lock, ChevronRight, X, RotateCcw, 
    Save, Map, BadgeCheck, Loader2
} from "lucide-react";
...
export function Profile() {
    ...
    if (loading && !user) return <div className="min-h-screen flex items-center justify-center"><Loader /></div>;
    if (!user) return null;
                ...
                <div className="lg:col-span-4 xl:col-span-3">
                    <Card className="bg-card-bg border-none shadow-sm overflow-hidden sticky top-24">
                        <CardContent className="p-8 flex flex-col items-center text-center">
                            ...
                            <h2 className="text-xl font-bold text-text-main mb-1 capitalize">{user.name}</h2>
                    ...
                    <Card className="bg-card-bg border-none shadow-sm">
                        ...
                        <CardContent className="p-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <InfoItem label="Full Name" value={user.name} icon={<User size={18} />} />
                                <InfoItem label="Email Address" value={user.email} icon={<Mail size={18} />} />
                                <InfoItem label="Contact Number" value={user.phone} icon={<Phone size={18} />} />
                                <InfoItem label="Location" value={user.location} icon={<MapPin size={18} />} />
                                <div className="md:col-span-2">
                                    <InfoItem label="Address" value={user.address} icon={<Map size={18} />} />
                                </div>
                            </div>

                            <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <SecurityCard icon={<Lock size={20} />} title="Security" desc="Manage account password" />
                                <SecurityCard icon={<BadgeCheck size={20} />} title="Status" desc="Active Super Admin" isGreen />
                            </div>
                        </CardContent>
                    </Card>
                </div>
                        ...
                        >
                            <div className="flex items-center justify-between p-6 border-b border-border-subtle bg-dashboard-bg/20">
                                <div className="flex items-center gap-3">
                                    <div className="p-2 bg-primary/20 rounded-lg"><User className="text-primary" size={20} /></div>
                                    <h3 className="text-lg font-bold text-text-main">Update Details</h3>
                                </div>
                                <button onClick={() => setIsEditModalOpen(false)} className="text-text-muted hover:text-white"><X size={20} /></button>
                            </div>

                            <div className="p-8 space-y-6 overflow-y-auto bg-card-bg custom-scrollbar">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <ModalInput label="Full Name *" name="name" value={formData.name} onChange={handleInputChange} icon={<User size={18} />} />
                                    <ModalInput label="Contact Number *" name="phone" value={formData.phone} onChange={handleInputChange} icon={<Phone size={18} />} />
                                    <div className="md:col-span-2">
                                        <ModalInput label="Location" name="location" value={formData.location} onChange={handleInputChange} icon={<MapPin size={18} />} />
                                    </div>
                                    <div className="md:col-span-2 space-y-1.5">
                                        <label className="text-[10px] font-bold text-text-muted uppercase ml-1">Address</label>
                                        <div className="relative">
                                            <div className="absolute left-4 top-4 text-text-muted opacity-50"><Map size={18} /></div>
                                            <textarea
                                                name="address"
                            ...
                            <div className="flex items-center justify-end gap-3 p-6 border-t border-border-subtle bg-dashboard-bg/50">
                                <button onClick={() => setIsEditModalOpen(false)} className="px-6 py-2 text-xs font-bold text-primary hover:underline flex items-center gap-1">
...
function InfoItem({ label, value, icon }) {
    return (
        <div className="space-y-2">
            ...
            <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted opacity-50">{icon}</div>
...
function ModalInput({ label, name, value, onChange, icon }) {
    return (
        <div className="space-y-1.5">
            ...
            <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted opacity-50">{icon}</div>
                <input type="text" name={name} value={value} onChange={onChange} className="w-full bg-dashboard-bg border border-border-subtle rounded-xl pl-12 pr-4 py-3 text-sm text-text-main focus:border-primary focus:outline-none" />
            </div>
        </div>
    );
}

function SecurityCard({ icon, title, desc, isGreen }) {
    return (
        <div className="p-6 rounded-2xl border border-border-subtle bg-dashboard-bg/50 flex flex-col">
            <div className={isGreen ? "text-green-500 mb-4" : "text-primary mb-4"}>{icon}</div>
