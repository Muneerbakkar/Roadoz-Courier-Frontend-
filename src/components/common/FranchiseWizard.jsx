...
export default function FranchiseWizard() {
  ...
  const inputClass = "w-full bg-dashboard-bg/50 border border-border-subtle rounded-lg px-4 py-2.5 text-sm text-text-main focus:outline-none focus:border-primary transition-all dark:bg-card-bg placeholder:text-text-muted/40";
  const labelClass = "text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5 block ml-1";
  ...
  const isStepComplete = () => {
    switch (currentStep) {
      ...
      case 4: return formData.investment_capacity && formData.bank_name && formData.account_number;
      case 5: return formData.preferred_service_area && formData.pin_codes_covered;
      case 6: return formData.doc_id_proof && formData.doc_address_proof && formData.doc_photographs;
      case 7: return formData.submission_place && formData.agree_to_terms;
  ...
  const handleSubmit = async () => {
    try {
      ...
      swalError("Error", err || "Submission failed.");
    }
  };
  ...
  if (id && loading && !selectedFranchise) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-text-muted font-bold animate-pulse uppercase tracking-widest text-center px-4">Fetching Franchise Records...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-20 pt-4 px-4 max-w-7xl mx-auto">
      ...
      <div className="flex overflow-x-auto gap-2 p-2 bg-card-bg border border-border-subtle rounded-2xl no-scrollbar shadow-sm">
        {STEPS.map((step) => {
          ...
          return (
            <div key={step.id} onClick={() => (isDone || step.id < currentStep) && setCurrentStep(step.id)} className={cn(
              "flex-1 min-w-[130px] flex items-center gap-3 p-3 rounded-xl transition-all cursor-pointer",
              isActive ? "bg-primary/10 border border-primary/20 shadow-inner" : isDone ? "opacity-100" : "opacity-50"
            )}>
              <div className={cn(
                "w-8 h-8 rounded-lg flex items-center justify-center border transition-all",
                isActive ? "bg-primary text-black border-primary scale-110 shadow-lg shadow-primary/30" : isDone ? "text-green-500 border-green-500" : "text-text-muted border-border-subtle"
              )}>
            ...
            {currentStep === 2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="md:col-span-2 border-b border-border-subtle pb-2 flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold text-text-main">2. Business Setup</h3>
                </div>
                <div>
            ...
            {currentStep === 6 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="border-b border-border-subtle pb-2 flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold text-text-main">6. Document Checklist</h3>
                </div>
