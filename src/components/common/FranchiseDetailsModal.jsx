...
export default function FranchiseDetailsModal({ isOpen, onClose, data }) {
  ...
  return (
    <CommonModal isOpen={isOpen} onClose={onClose} title={`Profile: ${data.franchise_code}`}>
      <div className="space-y-6 print:p-0" id="printable-area">
        ...
        <div className="bg-dashboard-bg p-6 rounded-2xl border border-border-subtle flex items-center justify-between">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h2 className="text-lg font-bold text-text-main leading-none">{data.full_name}</h2>
                 <p className="text-[10px] text-primary font-bold mt-1 uppercase tracking-wider">FRANCHISE CODE: {data.franchise_code}</p>
              </div>
           </div>
           <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase border ${data.is_active ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 px-2">
          ...
          <div className="md:col-span-2"><InfoField label="Current Address" value={data.current_address} /></div>
          ...
          <InfoField label="Years Active" value={data.years_active + " Years"} />
          <div className="md:col-span-2"><InfoField label="Detailed Business Address" value={data.detailed_business_address} /></div>

          <div className="md:col-span-2"><SectionHeader icon={HardHat} title="3. Infrastructure" /></div>
          <InfoField label="Office Space" value={data.office_space_sqft + " sq.ft"} />
          <InfoField label="Office Ownership" value={data.office_ownership} />
          <InfoField label="Staff Count" value={data.staff_count + " members"} />
          <InfoField label="Internet/Systems" value={`${data.internet_availability ? 'Internet Ready' : 'No Internet'} | ${data.computer_laptop ? 'PC Available' : 'No PC'}`} />

          <div className="md:col-span-2"><SectionHeader icon={IndianRupee} title="4. Financial Records" /></div>
          <InfoField label="Investment Capacity" value={data.investment_capacity} />
          ...
          <div className="md:col-span-2"><SectionHeader icon={Map} title="5. Territory & Docs" /></div>
          <InfoField label="Preferred Service Area" value={data.preferred_service_area} />
          <InfoField label="Landmark" value={data.nearby_landmark} />
          <div className="md:col-span-2 flex flex-wrap gap-2 mt-2">
            {['ID Proof', 'Address Proof', 'Photos', 'Business Reg', 'Bank Statement'].map((doc, i) => {
              const docKeys = ['doc_id_proof', 'doc_address_proof', 'doc_photographs', 'doc_business_registration', 'doc_bank_statement'];
              const isDone = data[docKeys[i]];
              return (
                <div key={i} className={`flex items-center gap-2 px-3 py-1.5 border rounded-lg text-[10px] font-bold ${isDone ? 'bg-dashboard-bg border-border-subtle text-text-muted' : 'bg-red-500/5 border-red-500/10 text-red-500/50'}`}>
                  <CheckCircle size={12} className={isDone ? "text-green-500" : "text-text-muted/20"} /> {doc}
                </div>
              );
            })}
          </div>
