              ...
              {isNotificationsOpen && (
                ...
                >
                    ...
                    {["Orders", "Wallet", "COD"].map((tab) => (
                      <button
                        ...
                        className={cn(
                          ...
                          activeNotifyTab === tab ? "text-primary border-primary" : "text-text-muted border-transparent"
                        )}
                      >
                        {tab}
                      </button>
                  ...
                  <div className="p-10 text-center bg-card-bg">
                    ...
                    <button className="text-primary text-[10px] font-bold uppercase mt-4 tracking-widest hover:underline">Mark all read</button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            >
              ...
              <div className="p-6 md:p-8 space-y-5 md:space-y-6">
                <div className="space-y-1.5">
                  ...
                  <select className="w-full bg-dashboard-bg border border-border-subtle rounded-xl px-4 py-3 text-sm text-text-main focus:border-primary outline-none"><option>B2C</option><option>B2B</option></select>
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-text-muted uppercase">Pickup Address *</label>
                  <select className="w-full bg-dashboard-bg border border-border-subtle rounded-xl px-4 py-3 text-sm text-text-main focus:border-primary outline-none"><option>#301 - Pickup - - 500049</option></select>
                </div>
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-col sm:flex-row border border-border-subtle rounded-xl overflow-hidden bg-dashboard-bg">
                    <label className="bg-primary/10 border-b sm:border-b-0 sm:border-r border-border-subtle px-6 py-3 text-xs font-bold text-primary cursor-pointer hover:bg-primary/20 text-center">Choose File <input type="file" className="hidden" /></label>
                    <span className="px-4 py-3 text-sm text-text-muted italic truncate">No file chosen</span>
                  </div>
                </div>
              ...
              <div className="p-6 border-t border-border-subtle bg-dashboard-bg/50 flex justify-end gap-3">
                ...
                <Button className="bg-primary text-black px-10 h-10 font-bold shadow-lg">Import</Button>
              </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              <div className="p-5 md:p-6 border-b border-border-subtle flex justify-between items-center">
                <h2 className="text-lg font-bold text-text-main">Recharge Your Wallet</h2>
              ...
              <div className="p-6 md:p-8 text-center space-y-6">
                ...
                <div className="space-y-4 text-left pt-2">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-text-muted uppercase">Amount (₹) *</label>
                    <input type="number" placeholder="0.00" className="w-full bg-dashboard-bg border border-border-subtle rounded-xl px-4 py-3 text-sm text-text-main focus:border-primary outline-none" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-text-muted uppercase tracking-wider">UTR / Transaction ID</label>
                    <input type="text" placeholder="Enter ID" className="w-full bg-dashboard-bg border border-border-subtle rounded-xl px-4 py-3 text-sm text-text-main focus:border-primary outline-none" />
                  </div>
                </div>
              </div>
              <div className="p-6 border-t border-border-subtle bg-dashboard-bg/50 flex justify-end gap-3">
                <button onClick={() => setIsWalletModalOpen(false)} className="px-6 py-2 text-sm font-bold text-text-muted">Cancel</button>
                <Button className="bg-primary text-black px-10 h-10 font-bold shadow-lg">Submit</Button>
              </div>
            </motion.div>
          </div>
        )}
            ...
            >
              <div className="p-6 md:p-10">
                <div className="flex justify-between items-center mb-8">
                  <h2 className="text-xl md:text-2xl font-bold text-text-main">Quick Actions</h2>
                  <button onClick={() => setIsQuickActionsOpen(false)} className="p-2 bg-dashboard-bg rounded-lg text-text-muted"><X size={24} /></button>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-border-subtle text-center">
                  <button onClick={() => setIsQuickActionsOpen(false)} className="text-sm font-bold text-text-muted hover:text-primary transition-colors uppercase tracking-widest">Close Action Menu</button>
                </div>
              </div>
            </motion.div>
          </div>
